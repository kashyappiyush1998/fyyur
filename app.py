...
def search_venues():
  ...
  venues = Venue.query.filter(func.lower(Venue.name).contains(search_term)).all()

  response = {"count": len(venues)}
  response['data'] = [{"id": venue.id,
                       "name": venue.name,
                       "num_upcoming_shows": 0} for venue in venues]
  print(response)
  # response={
  #   "count": 1,
  #   "data": [{
...
def create_venue_submission():
  ...
  try:
    ...
    venue = Venue(name=form_data['name'], city=form_data['city'], state=form_data['state'], 
                  address=form_data['address'], phone=form_data['phone'], image_link=form_data['image_link'],
...
def search_artists():
  ...
  search_term = request.form.get('search_term', '').lower()
  artists = Artist.query.filter(func.lower(Artist.name).contains(search_term)).all()

  response = {"count": len(artists)}
  response['data'] = [{"id": artist.id,
                       "name": artist.name,
                       "num_upcoming_shows": 0} for artist in artists]
  print(response)
...
def create_artist_submission():
  ...
  try:
    ...
    artist = Artist(name=form_data['name'], city=form_data['city'], state=form_data['state'], 
                    phone=form_data['phone'], genres= form_data['genres'], image_link=form_data['image_link'],
                    facebook_link=form_data['facebook_link'])
...
def create_show_submission():
  ...
  try:
    ...
    venue = Venue.query.get(venue_id)
    print(venue.name)
    show = Show(venue_id=venue_id, venue_name=venue.name, artist_id=artist_id, artist_name=artist.name, 
                artist_image_link=artist.image_link, start_time=start_time)
